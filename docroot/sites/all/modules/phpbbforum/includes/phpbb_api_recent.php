<?php
// $Id: phpbb_api_recent.php,v 1.5 2008/11/14 18:20:18 vb Exp $
/**
...
// Search
/*
function phpbb_api_search($num_recent = 10, $show_results = 'posts', $search_id = 'newposts', $topic_id = 0, $post_id = 0)
{
  ...
  $u_search .= ($topic_id) ? '&amp;t=' . $topic_id : '';
	...
	if ($sql_where)
	{
		...
		foreach ($rowset as $row)
		{
			...
			$view_topic_url = append_sid("{$phpbb_url}viewtopic.$phpEx", "f=$u_forum_id&amp;t=$result_topic_id&amp;hilit=$u_hilit");
      ...
      $posts[] = array_merge($tpl_ary, array(
				...
				'U_VIEW_FORUM'		=> append_sid("{$phpbb_url}viewforum.$phpEx", 'f=' . $forum_id),
				'U_VIEW_POST'		=> (!empty($row['post_id'])) ? append_sid("{$phpbb_url}viewtopic.$phpEx", "f=$forum_id&amp;t=" . $row['topic_id'] . '&amp;p=' . $row['post_id'] . '&amp;hilit=' . $u_hilit) . '#p' . $row['post_id'] : ''
			));
